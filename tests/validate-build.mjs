...
const talks = await readFile('dist/talks/index.html', 'utf8');
const news = await readFile('dist/updates/index.html', 'utf8');

assert.match(home, /<h1>Yunfei Ma<\/h1>/);
assert.match(home, /Global connectivity at Uber/);
...
assert.doesNotMatch(home, /keeps Uber moving/);
assert.match(home, /reliable, fast, cost-efficient, secure, and multi-cloud ready/);
assert.match(home, /Sr\. Engineering Manager/);
assert.match(home, /I lead Uber’s Mobile &amp; Edge Networking organization/);
assert.match(home, /manages all Layer 7 traffic into Uber/);
assert.match(home, /manages Uber’s global DNS infrastructure/);
assert.match(home, /across all Uber apps on iOS and Android/);
assert.match(home, /anti-DDoS infrastructure/);
assert.match(home, /network security/);
...
assert.match(home, /USENIX NSDI · Technical Program Committee/);
assert.match(home, /ACM MobiCom · Technical Program Committee/);
assert.match(home, /Uber Reimagine Award Finalist/);
assert.doesNotMatch(home, /Milestones, ideas, and field notes/);
...
assert.doesNotMatch(home, /I lead Uber’s Mobile, Edge, and Autonomous Vehicle Networking organization/);
...
assert.match(research, /<sup>\*<\/sup> Corresponding author/);
assert.equal((research.match(/aria-label="Yunfei Ma, corresponding author"/g) ?? []).length, 3);
assert.equal((research.match(/<sup aria-hidden="true">\*<\/sup>/g) ?? []).length, 3);
...
assert.match(about, /manages Uber’s global DNS infrastructure/);
assert.match(about, /Mobile &amp; Edge Networking/);
assert.doesNotMatch(about, /Mobile, edge &amp; autonomous vehicle networking/);
...
assert.match(about, /Selected coverage/);
